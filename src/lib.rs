...
impl Iface {
    ...
    fn with_options(ifname: &str, mode: Mode, packet_info: bool) -> Result<Self,Error> {
        if packet_info {
            return Err(Error::new(ErrorKind::Other, "packet info not yet supported"))
        }
        let c_ifname = CString::new(ifname).expect("CStringe::new failed");
        ...
        let name_ptr: *mut c_char = name_buffer.as_mut_ptr() as *mut i8;
        let result = unsafe {
